...
const MOCK_AI_REVIEW = {
    mustFix: [
        {
            id: 1,
            file: "src/services/pay.service.ts",
            line: 42,
            title: "필수 수정 (Must Fix)",
            content: "결제 트랜잭션 처리 중 예외 발생 시 롤백 로직이 누락되었습니다. try-catch 블록을 추가하여 데이터 무결성을 보장해주세요.",
            keyword: "try-catch",
    ...
    recommendation: [
        {
            ...
            line: 12,
            title: "스타일 (Style)",
            content: "변수명 tempData는 역할이 불분명합니다. paymentHistory와 같이 구체적인 네이밍을 권장합니다.",
            keyword: "tempData",
            status: "pending"
        }
    ]
                            ...
                            <div className="col-span-7 space-y-16">
                                <section>
                                    ...
                                    <div className="space-y-0">
                                        {/* Step 1 */}
                                        <div className={`flex gap-8 transition-opacity duration-300 ${currentStep === 1 ? 'opacity-100' : 'opacity-40'}`}>
                                            <div className="flex flex-col items-center pt-1 shrink-0">
                                                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-lg ${currentStep === 1 ? 'bg-[#3182F6] text-white' : 'bg-white text-[#8B95A1] border-2 border-[#E5E8EB]'}`}>
                                                    {currentStep > 1 ? <i className="fa-solid fa-check"></i> : 1}
                                                </div>
                                                <div className="w-0.5 flex-1 bg-[#E5E8EB] my-2"></div>
                                            </div>
                                            <div className="pb-12">
                                                <h3 className="text-lg font-bold text-[#191F28] mb-3">Github Bot 설치</h3>
                                                ...
                                                {currentStep === 1 && (
                                                    ...
                                                    >
                                                        {isLoading ? "설치 URL 가져오는 중..." : "Github Bot 설치하기"} <i className="fa-solid fa-arrow-right"></i>
                                                    </button>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {/* Step 2 */}
                                        <div className={`flex gap-8 transition-opacity duration-300 ${currentStep === 2 ? 'opacity-100' : 'opacity-40'}`}>
                                            <div className="flex flex-col items-center pt-1 shrink-0">
                                                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-lg ${currentStep === 2 ? 'bg-[#3182F6] text-white' : 'bg-white text-[#8B95A1] border-2 border-[#E5E8EB]'}`}>
                                                    {currentStep > 2 ? <i className="fa-solid fa-check"></i> : 2}
                                                </div>
                                                <div className="w-0.5 flex-1 bg-[#E5E8EB] my-2"></div>
                                            </div>
                                            <div className="pb-12">
                                                <h3 className="text-lg font-bold text-[#191F28] mb-2">코드 작성 및 PR 생성</h3>
                                                <p className="text-[#4E5968] text-sm mb-4">제시된 이슈를 수정하고 PR 생성 후 병합을 진행해주세요.</p>
                                                {currentStep === 2 && (
                                        ...
                                        {/* Step 3 */}
                                        <div className={`flex gap-8 transition-opacity duration-300 ${currentStep === 3 ? 'opacity-100' : 'opacity-40'}`}>
                                            <div className="flex flex-col items-center pt-1 shrink-0">
                                                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-lg ${currentStep === 3 ? 'bg-[#3182F6] text-white' : 'bg-white text-[#8B95A1] border-2 border-[#E5E8EB]'}`}>
                                                    3
                                                </div>
                                            </div>
                                            <div>
                                                <h3 className="text-lg font-bold text-[#191F28] mb-2">AI 코드 리뷰 확인</h3>
                                                <p className="text-[#4E5968] text-sm">AI 사수가 분석한 코드 리뷰 결과를 확인하세요.</p>
                                            </div>
                                        </div>
                                    </div>
                                </section>
                                    ...
                                    <section className="animate-fade-in-up">
                                        <div className="flex items-center justify-between mb-8">
                                            ...
                                            <div className="flex gap-3">
                                                <span className="flex items-center gap-2 px-4 py-1.5 bg-[#FFF0F0] text-[#F04452] text-xs font-bold rounded-full">
                                                    <i className="fa-solid fa-circle-exclamation"></i> 필수 수정 {MOCK_AI_REVIEW.mustFix.length}
                                                </span>
                                                <span className="flex items-center gap-2 px-4 py-1.5 bg-[#FFF8E1] text-[#FFBB00] text-xs font-bold rounded-full">
                                                    <i className="fa-solid fa-circle-info"></i> 권장 {MOCK_AI_REVIEW.recommendation.length}
                                                </span>
                                            </div>
                                        </div>
                                        <div className="space-y-6">
                                            ...
                                            {MOCK_AI_REVIEW.mustFix.map((item) => (
                                                <div key={item.id} className="group p-8 bg-white rounded-[32px] transition-all hover:translate-x-1 border border-transparent hover:border-[#F04452]/20">
                                                    <div className="flex justify-between items-start mb-4">
                                                        ...
                                                        <span className="text-xs font-mono text-[#8B95A1]">{item.file}:{item.line}</span>
                                                    </div>
                                                    <p className="text-[#333D4B] font-medium leading-relaxed mb-6">
                                                        {item.content.split(item.keyword).map((part, i, arr) => (
                                                            <React.Fragment key={i}>
                                                                {part}
                                                                {i < arr.length - 1 && (
                                                                    <code className="bg-[#F2F4F6] px-1.5 py-0.5 rounded text-[#F04452]">{item.keyword}</code>
                                                                )}
                                                            </React.Fragment>
                                                        ))}
                                                    </p>
                                            ...
                                            {/* Recommendation Items */}
                                             {MOCK_AI_REVIEW.recommendation.map((item) => (
                                                <div key={item.id} className="group p-8 bg-white rounded-[32px] transition-all hover:translate-x-1">
                                                    <div className="flex justify-between items-start mb-4">
                                                    ...
                                                    <p className="text-[#333D4B] font-medium leading-relaxed mb-6">
                                                         {item.content.split(item.keyword).map((part, i, arr) => (
                                                            <React.Fragment key={i}>
                                                                {part}
                                                                {i < arr.length - 1 && (
                                                                    <code className="bg-[#F2F4F6] px-1.5 py-0.5 rounded text-[#3182F6]">{item.keyword}</code>
                                                                )}
                                                            </React.Fragment>
                                                        ))}
                                                    </p>
                            ...
                            <div className="col-span-5 space-y-16">
                                <section>
                                    ...
                                    <div className="space-y-8">
                                        ...
                                        <div className="flex items-center gap-5">
                                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${currentStep >= 2 ? 'bg-[#E8F3FF] text-[#3182F6]' : 'bg-[#F2F4F6] text-[#8B95A1]'}`}>
                                                <i className="fa-solid fa-code-pull-request text-xl"></i>
                                            </div>
                                            <div className="flex-1">
                                                <div className="text-base font-bold text-[#191F28]">PR 생성하기</div>
                                                <div className={`text-sm font-medium ${currentStep >= 3 ? 'text-[#2DCA73]' : currentStep === 2 ? 'text-[#3182F6]' : 'text-[#8B95A1]'}`}>
                                                    {currentStep >= 3 ? 'Merged' : currentStep === 2 ? 'In Progress...' : '리뷰 반영 후 활성화됩니다'}
                                                </div>
                                            </div>
                                            {currentStep >= 3 ? (
                                                <i className="fa-solid fa-circle-check text-[#2DCA73] text-xl"></i>
                                            ) : (
                                                <button className={`px-5 py-2 text-xs font-bold rounded-xl transition-colors ${currentStep === 2 ? 'bg-[#3182F6] text-white hover:bg-[#2272EB]' : 'bg-[#F2F4F6] text-[#8B95A1] cursor-not-allowed'}`}>
                                                    작성
                                                </button>
                                            )}
                                        </div>
